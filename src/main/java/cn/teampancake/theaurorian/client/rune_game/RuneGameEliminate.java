...
public class RuneGameEliminate {
    ...
    public void eliminate() {
        Map<String, List<RuneGameBrand>> collect = slots.stream().collect(
                Collectors.groupingBy((RuneGameBrand brand) -> brand.getTexture().toString()));
        collect.forEach((key, value) -> {
            if (value.size() >= 3) {
                this.slots.removeIf(next -> next.getTexture().toString().equals(key));
            }
        });
    }
