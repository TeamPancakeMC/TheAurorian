...
public class WaterRunestone extends Runestone {

    public WaterRunestone(Properties properties, int rank) {
        super(properties, rank);
    }

    @Override
    public List<Component> getAttributesTooltip(List<Component> tooltips, TooltipContext context, ItemStack stack) {
        RunestoneWater runestoneWater = stack.get(TADataComponents.RUNESTONE_WATER);
        Integer rank = stack.get(TADataComponents.RANK);
        if (runestoneWater != null && rank != null) {
            String key = "attribute.theaurorian.name.player.water_runestone";
