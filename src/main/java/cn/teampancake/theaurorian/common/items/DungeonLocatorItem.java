...
import cn.teampancake.theaurorian.utils.GenerationHelper;
...
public class DungeonLocatorItem extends Item implements ITooltipsItem{
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand pUsedHand) {
        ...
        if (player.isCrouching()) {
            ...
            BlockPos dungeon = switch (this.getSelectedDungeon(itemstack)) {
                case "Moontemple" ->
                        GenerationHelper.getNearestStructure(new MoonTempleWorldGenerator(), player, AurorianConfig.CONFIG_DUNGEON_DENSITY.get() * 4);
                default ->
                        GenerationHelper.getNearestStructure(new RunestoneTowerWorldGenerator(), player, AurorianConfig.CONFIG_DUNGEON_DENSITY.get() * 2);
                case "Darkstone" ->
                        GenerationHelper.getNearestStructure(new DarkstoneDungeonWorldGenerator(), player, AurorianConfig.CONFIG_DUNGEON_DENSITY.get() * 6);
            };
