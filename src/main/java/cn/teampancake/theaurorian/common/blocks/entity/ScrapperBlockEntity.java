...
public class ScrapperBlockEntity extends SimpleContainerBlockEntity implements WorldlyContainer {
    ...
    private final RecipeManager.CachedCheck<SingleRecipeInput, ? extends ScrapperRecipe> quickCheck;
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, ScrapperBlockEntity blockEntity) {
        if (!level.isClientSide()) {
            ...
            ScrapperRecipe recipe = !ingredient.isEmpty() ? blockEntity.quickCheck.getRecipeFor(new SingleRecipeInput(ingredient), level).orElse(null).value() : null;
    ...
    protected void startWork(RegistryAccess registryAccess, @Nullable Recipe<SingleRecipeInput> recipe, NonNullList<ItemStack> inventory, int maxStackSize) {
        if (recipe != null && this.canWork(registryAccess, recipe, inventory, maxStackSize)) {
            SingleRecipeInput input = new SingleRecipeInput(inventory.get(0));
    ...
    protected boolean canWork(RegistryAccess registryAccess, @Nullable Recipe<SingleRecipeInput> recipe, NonNullList<ItemStack> inventory, int maxStackSize) {
