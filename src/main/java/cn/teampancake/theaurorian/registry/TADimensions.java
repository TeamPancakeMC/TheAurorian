...
public class TADimensions {
    ...
    public static void bootstrapNoise(BootstapContext<NoiseGeneratorSettings> context) {
        ...
        NoiseGeneratorSettings settings = new NoiseGeneratorSettings(noiseSettings, TABlocks.AURORIAN_STONE.get().defaultBlockState(),
    ...
    private static SurfaceRules.RuleSource createSurfaceRule() {
        ...
        SurfaceRules.RuleSource forestRules = createNormalRuleSource(TABiomes.AURORIAN_FOREST, TABlocks.AURORIAN_GRASS_BLOCK.get());
        SurfaceRules.RuleSource plainRules = createNormalRuleSource(TABiomes.AURORIAN_PLAINS, TABlocks.AURORIAN_GRASS_BLOCK.get());
        SurfaceRules.RuleSource lakeRules = createNormalRuleSource(TABiomes.AURORIAN_LAKES, TABlocks.AURORIAN_GRASS_BLOCK.get());
        SurfaceRules.RuleSource weepingWillowForestRules = createNormalRuleSource(
                TABiomes.WEEPING_WILLOW_FOREST, TABlocks.AURORIAN_GRASS_LIGHT_BLOCK.get());
        ...
        return SurfaceRules.ifTrue(SurfaceRules.isBiome(resourceKey), SurfaceRules.ifTrue(SurfaceRules.abovePreliminarySurface(),
                SurfaceRules.sequence(SurfaceRules.ifTrue(SurfaceRules.ON_FLOOR, SurfaceRules.sequence(
                                SurfaceRules.ifTrue(SurfaceRules.waterBlockCheck(-1, ConstantInt.ZERO.getValue()),
                                        SurfaceRuleData.makeStateRule(surfaceBlock)), SurfaceRuleData.makeStateRule(TABlocks.AURORIAN_DIRT.get()))),
                        SurfaceRules.ifTrue(SurfaceRules.UNDER_FLOOR, SurfaceRuleData.makeStateRule(TABlocks.AURORIAN_DIRT.get())))));
    }
